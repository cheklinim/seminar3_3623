...
Есть два варианта работы с чужим удаленным репозиторием, но в первом и во втором случае вам необходимо его клонировать.

* Сначала заходим на чужой удаленый репозиторий 
* Нажимаем кнопку *code* и копируем ее в буфер обмена
* Переходим в Visual Code и в путой папке без репозитория прописываем следующие команды:

        git clone <ссылка на репозиторий>

Теперь нужно зайти в сам перозиторий:

        cd .\<файл с репозиторием>

 Проверяем с помощью команды *git status*

 Таким образом вы можете просто посмотреть чужой репозиторий и менять его, но если вам хочется предложить изменения автору репозитория, то делаем следующее.

 * В главной странице репозитория у автора используем кнопку *fork*
 * Этот репозиторий появляется уже в вашем списке репозиториев. Повторяем вышеописанные действия с копированием.

 Особенности внесения изменения - все ваши изменения необходимо производить в дополнительной ветки, не используя главную (Это внегласные правила).

 После внесения изменений сихронизируем ваш локальный репозиторий и удаленый при помощи команды : *git push*      

 Если вы будете выгружать из второстепенной ветки, то git предложит правильную команду, используйте ее.

 После того, как вы синхронизировали репозитории, то вы можете предложить изменения автору путем нажатия кнопки **Create pull request**

 Обычно еще добавляют файл **README.md** в котором подробно описываются ваши изменения.