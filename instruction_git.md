...
Для того, чтобы начать работу с чужим репозиторием, нужно сделать его копию на своем аккаунте с помощью команды:

    Fork

Далее порядок действий будет следующий: 

* Клонируем собственный публичный форк репозитория. Это можно сделать с помощью знакомой команды git clone.

* Ставим нужную папку репозитория для работы в VS Code (cd)
...
* На сайте GitHub нажимаем кнопку pull request, если требуется, пишем сообщение автору

В конечном итоге автор репозитория увидит наше сообщение с предложенным вариантом проекта и решит принимать полученное от нас предложение или отказать.