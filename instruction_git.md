...
Используется для совместной удаленной работы.

### Связь локального и удаленного репозитория

Создаем новый репозиторий в аккаунте github. Выбираем способ «…or push an existing repository from the command line». Запускаем его из командной строки в VSCode.

В командной строке последовательно вводим следующие команды:

1. git remote add origin https://  ссылка_на наш репозиторий

2. git branch -M main (при необходимости заменить main на master)

3. git push -u origin main (при необходимости заменить main на master)

После третей команды git push, связь между удаленным и локальным репозиторием будет подключена.

### Перенос содержимого локального репозитория в удаленный
...
### Перенос содержимого удаленного репозитория в локальный
...
Предложение изменения кода в чужом репозитории осуществляется через:

    pull request (пул-реквест) - PR (пиар, он же merge request MR(мр))

Допустим, вы забрали к себе чужой репозиторий, поработали с ним и теперь хотите, чтобы ваши изменения попали в оригинальный репозиторий — тогда вы создаёте создаёте PR с просьбой добавить ваши изменения в репозиторий.
