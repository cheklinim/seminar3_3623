...
При работе в больших компаниях, где один ответственный за проект создает аккаунт проекта, в который предполагается, что другие пользователи будут вносить изменения, или например, когда проект с открытым исходным кодом (opensource project), означающий, что любой может свободно использовать, изучать, модифицировать и распространять этот проект для любых целей через специальную лицензию с открытым исходным кодом. В таких случаях используется следующий порядок работы с удаленным репозиторием:

* создается fork (ответвление) репозитория, при котором уже на нашем удаленном аккаунте, например на GitHub.com создается локальный репозиторий, являющийся точной копией исходника

* клонируется эта локальная версия репозитория с удаленного аккаунта на наш ПК

* создается новая ветка в которую будут вноситься предполагаемые изменения

* внесенные в эту новую ветку изменения отправляются в наш удаленный аккаунт командой push

* уже в нашем удаленном локальном репозитории делается запрос на вливание в репозиторий проекта или того автора, которому мы хотим предложить наши изменения и над репозиторием которого мы делали fork. Для предложения изменений используется следующая команда на GitHub:
